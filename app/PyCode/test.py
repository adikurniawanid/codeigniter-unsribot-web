...
cleanWord = [w for w in sub(
    r"[^\w\s]", "", text).lower().split() if w not in stopwords]

print(f"""
input               : {text}
case folding        : {text.lower()}
hilang tanda baca   : {var_masukan.lower()}
stemming            : {var_masukan}
token               : {[w for w in var_masukan.lower().split()]}
Stopword removal    : {cleanWord}
NER                 :
Query Form          :
""")
